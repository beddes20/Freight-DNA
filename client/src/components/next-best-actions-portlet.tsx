 ...
 * Follows the same collapsible/header pattern as AccountGrowthPortlet.
 */
...
interface NbaItem {
  ...
  ctaLabel:    string;
  signals: {
    // companyId is not on the signals object — the batch endpoint must attach it
    [key: string]: unknown;
  };
  // Batch endpoint attaches these for navigation
  companyId:   string;
  companyName: string;
}
    ...
    <Card data-testid="portlet-next-best-actions">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold">Priority Actions Today</span>
          {!isLoading && !isError && (
            <div className="flex items-center gap-1.5">
              {criticalCount > 0 && (
                <Badge
                  ...
                  data-testid="badge-nba-critical-count"
                >
              ...
              {highCount > 0 && (
                <Badge
                  className="text-xs px-1.5 bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 border-0"
                  data-testid="badge-nba-high-count"
                >
                  {highCount} high
                </Badge>
              )}
              {urgentCount === 0 && items.length > 0 && (
                <Badge
                  className="text-xs px-1.5 bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 border-0"
                  data-testid="badge-nba-moderate-count"
                >
      ...
      {!collapsed && (
        <CardContent className="pt-0 px-4 pb-4">
          ...
          {/* Empty — all accounts in good shape */}
          {!isLoading && !isError && items.length === 0 && (
            <div
              className="flex flex-col items-center gap-2 py-6 text-muted-foreground"
              data-testid="nba-portlet-empty"
            >
              ...
              {items.map((item, idx) => {
                ...
                return (
                  ...
                  >
                    {/* Urgency dot */}
                    <span
                      className={`shrink-0 h-2 w-2 rounded-full ${u.dot}`}
                      aria-label={u.label}
                    />
                    ...
                    {/* Urgency badge */}
                    <span
                      className={`shrink-0 text-[10px] font-medium px-1.5 py-0.5 rounded border ${u.badge}`}
                    >
                      {u.label}
                    </span>
