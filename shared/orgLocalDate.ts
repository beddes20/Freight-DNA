/**
 * Org-local "today" helpers (shared between server and client).
 *
 * The Available Freight stack runs on US Central time — the importer is
 * scheduled in CT and reps triage on the CT business day. UTC midnight
 ...
 *
 * NOTE: lives in `shared/` so the cockpit client filter, the cockpit server
 * SQL aggregates, and the server route can all anchor on the same definition
 * of "today" — see task #875.
 */
