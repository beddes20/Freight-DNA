...
-   **Quote Requests post-2d UX/IA spec + backend contract (planning artifacts, not yet implemented)**: `docs/quote-requests-tab-post-2d-spec.md` is the UX/IA design contract for the operator surface that replaces `customer-quotes.tsx` once Phase 2b (forward closure), 2c (capture-leak auto-empty), and 2d (legacy fold-in) are stable; pins down the system-of-record contract (one row = one `quote_opportunities`), list/detail/empty states, action catalog, keyboard nav, cross-surface boundaries, and OPEN questions. `docs/quote-requests-tab-post-2d-backend-contract.md` is the locked backend implementation contract for Task #849: resolves the 3 BLOCKING schema decisions (extend `QUOTE_SOURCES` with `email_signal`/`spot_search`; extend `CAPTURE_LEAK_REVIEW_DECISIONS` with `returned_to_queue`/`duplicate`/`not_a_request`; add `'attached'` to `QUOTE_OUTCOME_STATUSES`); defines 5 new endpoints with full request/response shapes (`POST .../quote/:id/attach-to`, `POST .../quote/:id/send-to-leak`, `PATCH .../quote/:id/snooze`, `GET /api/quote-requests/automation-counters`, `POST /api/email-conversations/:threadId/reply`); locks the disposition state model, event-writing rules, permission-by-role matrix, and canonical-vs-denormalized field map; and lays out an 8-slice sprint plan with per-endpoint acceptance criteria that unblocks Task #850 (UI build). Three Canvas mockups in `artifacts/mockup-sandbox/src/components/mockups/quote-requests-post-2d/` — `PopulatedList.tsx`, `RowAndDetailDrawer.tsx`, `EmptyState.tsx` — visualize the populated list, detail drawer, and empty state.
