...
-   **Conversation Threads `updated_at` Contract (Task #860)**: `email_conversation_threads.updated_at` advances ONLY on real conversation events a rep would care about — a new email landed (`upsertEmailConversationThread` / `applyMessageToThread`), or owner / waiting-state / priority / manual snooze flipped via a user action (`updateEmailConversationThread`, `routes/emailAnalytics.ts` owner reassignment, user-initiated `wakeSnoozedThread`). Background sweeps and scheduler-driven state changes — auto-archive cron (`conversationArchiveScheduler.ts`), classification reclassify (`conversationThreadBackfillService.ts`), and the snooze-expiry wake (`wakeSnoozedThreadInternal` invoked from `wakeExpiredSnoozes`) — route through `storage.touchEmailConversationThreadInternal` (or write `row_version_at` directly in raw SQL sweeps) and bump only the separate `row_version_at` audit clock. The user-vs-scheduler split is enforced by two distinct exported wake entry points (`wakeSnoozedThread` for the rep-clicked unsnooze endpoint; `wakeSnoozedThreadInternal` for the cron). Section 24 of `tests/code-quality-guardrails.test.ts` (14 assertions) pins the schema column, migration column + seed, sweep call sites, the internal-touch storage method's SET clause, the wake function split, and the scheduler's call site. The column is materialized + seeded by an idempotent migration in `runMigrations.ts`.
