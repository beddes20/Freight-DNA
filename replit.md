...
-   **Quote Requests post-2d UX/IA spec (planning artifact, not yet implemented)**: `docs/quote-requests-tab-post-2d-spec.md` is the design contract for the operator surface that replaces `customer-quotes.tsx` once Phase 2b (forward closure), 2c (capture-leak auto-empty), and 2d (legacy fold-in) are stable. Pins down the system-of-record contract (one row = one `quote_opportunities` row), list/detail/empty/loading/error states, full action catalog with API contracts (existing endpoints reused where possible; identifies new endpoints `POST /api/customer-quotes/quote/:id/send-to-leak`, `PATCH /api/customer-quotes/quote/:id/snooze`, `GET /api/quote-requests/automation-counters`), keyboard navigation, cross-surface boundaries (Quote Requests vs Conversations vs Capture Leak Queue vs legacy Customer Quotes redirect), and an explicit OPEN-questions section. Three Canvas mockups in `artifacts/mockup-sandbox/src/components/mockups/quote-requests-post-2d/` — `PopulatedList.tsx`, `RowAndDetailDrawer.tsx`, `EmptyState.tsx` — visualize the populated list, detail drawer, and empty state.
