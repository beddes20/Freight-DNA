...
- **Core Features**:
    ...
    - **Win/Loss Pattern Dashboard** (Email Intelligence): New analytics page (`/email-intelligence`) surfacing org-wide email signal patterns. Four tabs: Urgency Tracker (unresponded urgency signals with elapsed time + touchpoint response status), Win/Loss Patterns (recharts bar chart showing which signal types precede won/lost deals via `email_outcome_links`), Signal Overview (top intent type volume + individual signal cards), and Recent Feed (last 30 signals with click-through to accounts/carriers). API: `GET /api/analytics/email-intelligence`. Accessible to admin, director, NAM, sales_director roles. Sidebar nav item added with `BrainCircuit` icon.
    - **Carrier Reliability Score** (Carrier Hub): `/api/carrier-hub/:id/activity` now computes per-carrier reliability metrics: `repliesReceived` (inbound logs where `matched_carrier_id` matches), `replyRate` (%), `softCommitments`, `hardCommitments`, `laneOffers`, `laneDeclines`, `hardCommitmentRate`, and `reliabilityScore` (0–100, weighted: reply rate 40% + hard commitment rate 30% + positive outcome rate 30%). Displayed in the Carrier Hub profile as a color-coded Reliability Score panel (green ≥70, amber 40–69, red <40) with a progress bar. Panel appears when Lanes or Activity tab is clicked.
    - **Urgency Response Tracker**: Urgency signals (`urgency_signal` intent type) detected in customer emails are tracked via the Email Intelligence page's Urgency Tracker tab. Shows which accounts have unresponded urgency signals (no touchpoint logged after signal), with hours-elapsed display and color-coded status (Urgent <4h, Needs Attention <24h, Overdue ≥24h, Responded). NBA cards for urgency signals are already generated by `generateAccountEmailNbas` (maps to `overdue_next_action` rule type, urgency "high").
